...
def main():
    ...
    try:
        ...
        # Get all professor names from the database
        cursor.execute("SELECT name FROM professors")
        professors = [row[0] for row in cursor.fetchall()]
        
        if not professors:
            print("No professors found in the database. Cannot add courses without professors.")
            return
        
        print(f"Found {len(professors)} professors in the database.")
        ...
        for course_name in FIESC_COURSES:
            ...
            if not existing_course:
                # Randomly assign a professor to the course
                ...
                description = f"This course covers topics in {course_name}"
                
                # Insert new course
                cursor.execute(
                    """
                    INSERT INTO courses 
                    (id, name, credits, profesor_name, year, semester, description, faculty_id) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """,
                    (next_course_id, course_name, credits, professor_name, year, semester, description, faculty_id)
                )
                conn.commit()
                added_courses += 1
                print(f"Added course: {course_name} (ID: {next_course_id}, Professor: {professor_name})")
            else:
        ...
        cursor.execute("SELECT id, name, profesor_name FROM courses WHERE faculty_id = %s", (faculty_id,))
        ...
        for course in fiesc_courses:
            ...
            print(f"  - {course_name} (ID: {course_id}, Professor: {professor})")
