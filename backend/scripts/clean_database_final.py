...
This script uses direct SQL queries with the correct column names
"""
import sys
import os
...
def clean_database_with_sql(external_data):
    """
    Clean the database using direct SQL queries with the correct column names
    """
    db = next(get_db())
    try:
        print("\nCleaning the database...")

        # For professors, we'll use the name as the key since there's no direct ID mapping
        professor_names = [p.get('firstName', '') + ' ' + p.get('lastName', '') for p in external_data['professors']]
        professor_names = [name.strip() for name in professor_names if name.strip()]
        
        # For rooms, we'll use the name as the key
        room_names = [r.get('name', '') for r in external_data['rooms']]
        room_names = [name.strip() for name in room_names if name.strip()]
        
        # For faculties, we'll use the ID as the key
        faculty_ids = [str(f.get('id', '')) for f in external_data['faculties']]
        faculty_ids = [fid for fid in faculty_ids if fid]
        
        # For groups, we'll use the name as the key
        group_names = [g.get('name', '') for g in external_data['subgroups']]
        ...
        if professor_names:
            # Create a SQL string with all professor names
            professor_names_sql = "', '".join(professor_names)
        ...
        if room_names:
            room_names_sql = "', '".join(room_names)
            if room_names_sql:
        ...
        if group_names:
            group_names_sql = "', '".join(group_names)
            if group_names_sql:
        ...
        if faculty_ids:
            faculty_ids_sql = "', '".join(faculty_ids)
            if faculty_ids_sql:
