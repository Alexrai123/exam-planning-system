    ...
    try:
        with engine.connect() as conn:
            ...
            bfkt_year1_courses = [
                ...
                {"name": "Obiectul Anatomiei. Istoric. Nomenclatura anatomica", "year": 1, "semester": 1},
                ...
                {"name": "Anatomie descriptiva. Vascularizatia venoasa a membrului superior", "year": 1, "semester": 2},
            ...
            # List all courses for BFKT Year 1
            result = conn.execute(text(
                """
                SELECT id, name, year, semester 
                FROM courses 
                WHERE faculty_id = '21' AND year = 1
                ORDER BY semester, id
                """
            ))
            rows = result.fetchall()
            
            print(f"\nBFKT Year 1 courses in database ({len(rows)}):")
            print("-" * 80)
