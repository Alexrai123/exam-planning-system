...
def add_fiesc_faculty_and_specializations():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Define FIESC specializations from the provided image
            ...
            result = conn.execute(text(
                """
                SELECT f.id, f.name, COUNT(s.id) as spec_count
                FROM faculties f
            ...
            print("-" * 80)
            print(f"{'Faculty ID':<12} {'Faculty Name':<50} {'Specialization Count':<20}")
            print("-" * 80)
            
            for row in faculty_counts:
                print(f"{row[0]:<12} {row[1]:<50} {row[2]:<20}")
