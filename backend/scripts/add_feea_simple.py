...
def add_feea_faculty_and_specializations():
    """
    Add FEEA faculty and specializations to the database
    """
    try:
        with engine.connect() as conn:
            # First check if FEEA faculty exists
            ...
            if feea_faculty:
                ...
                print(f"FEEA faculty not found, creating it with ID: {next_id}...")
                conn.execute(text(
                    "INSERT INTO faculties (id, name, short_name) VALUES (:id, :name, :short_name)"
                ), {
                    "id": next_id,
                    "name": "Facultatea de Economie si Administrarea Afacerilor",
                    "short_name": "FEEA"
                })
                conn.commit()
                
                feea_id = next_id
                print(f"Created FEEA faculty with ID: {feea_id}")
            
            # Define FEEA specializations
        ...
        print(f"Error adding FEEA faculty and specializations: {e}")

if __name__ == "__main__":
    add_feea_faculty_and_specializations()
